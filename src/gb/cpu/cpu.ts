...
export class CPU {
  ...
  /** Cycles spent in HALT / STOP since the last reset. Reset by
   *  `runFrame` each visible frame so the host can compute a per-frame
   *  "GB CPU load" metric (`1 - haltedCycles / totalCycles`). Transient
  ...
  step(): number {
    ...
    // Capture IME as it stood ENTERING this step, then promote a deferred
    // EI. The pending-interrupt check below uses `effectiveIme`, preserving
    // EI's documented 1-instruction delay (the instruction right after EI
    // can't have its IRQ serviced here). But `this.ime` is now updated so
    // the HALT opcode in `execute()` sees the post-EI IME state — required
    // by SameSuite `ei_delay_halt`: `EI; HALT` with a buffered IRQ should
    // dispatch the IRQ via the normal path with PC pointing back at HALT
    // (re-entering on RETI), not exit HALT via the wake path.
    ...
    if (pendingMask !== 0) {
      if (this.halted) {
        ...
        // NEXT step() call.
        this.halted = false;
        this.haltedCycles++; // the wake cycle counts as halted time
        this.finishTicks(1);
        return 1;
      }
      if (effectiveIme) {
  ...
  private execute(): number {
    ...
    if (op >= 0x40 && op <= 0x7f) {
      if (op === 0x76) {
        // HALT with pending IRQ has two distinct quirks per IME state:
        // - IME=0: DMG "HALT bug" — skip halting; the next fetch reads the
        //   same byte twice (`haltBug` is consumed by `fetchByte`).
        // - IME=1: don't halt; rewind PC so the immediately-dispatched IRQ
        //   pushes the HALT address and RETI returns to re-execute HALT.
        //   SameSuite `ei_delay_halt` requires this: buffered IRQs are
        //   serviced from inside HALT without exiting it.
        const pending = this.interrupts.ie & this.interrupts.if & 0x1f;
        if (pending !== 0) {
          if (this.ime) this.regs.pc = (this.regs.pc - 1) & 0xffff;
