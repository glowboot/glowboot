...
import { STATE_VERSION, StateReader, StateWriter, UnsupportedSaveStateError, upgradeState } from "./serialization.js";
...
describe("upgradeState", () => {
  it("returns blobs at the current version unchanged", () => {
    const blob = new Uint8Array([STATE_VERSION, 0xaa, 0xbb]);
    const out = upgradeState(blob);
    expect(out).toBe(blob); // no copy when no migration is needed
  });

  it("rejects an empty blob with UnsupportedSaveStateError", () => {
    expect(() => upgradeState(new Uint8Array())).toThrow(UnsupportedSaveStateError);
  });

  it("rejects a future-version blob (newer than this build)", () => {
    const blob = new Uint8Array([STATE_VERSION + 1, 0xff]);
    expect(() => upgradeState(blob)).toThrow(UnsupportedSaveStateError);
  });

  it("rejects an older-than-oldest blob (no migrator registered)", () => {
    if (STATE_VERSION === 1) {
      // v0 → no migrator at v0; this is the realistic "very old save" case.
      expect(() => upgradeState(new Uint8Array([0]))).toThrow(UnsupportedSaveStateError);
    }
  });
});
