...
describe("SquareChannel", () => {
  ...
  describe("duty cycle", () => {
    it("outputs the 12.5% duty waveform for NR*1 bits 6-7 = 00", () => {
      writeReg(ch, 1, 0x00); // duty 0
      writeReg(ch, 2, 0xf0); // vol 15
      writeReg(ch, 3, 0xff); // NR*3: freqLo = 0xFF
      writeReg(ch, 4, 0x87); // NR*4: trigger + freqHi=7 → freqReg = 2047 (period 4 T)

      // HW duty 0 = 00000001 sampled MSB→LSB across steps 0..7, so step 0..6
      // are LOW and step 7 is HIGH. After trigger dutyPos = 0, so sample = 0.
      expect(ch.sample()).toBe(0);
      // Step the duty 7 times. Each step is 4 T-cycles at freqReg=2047.
      for (let i = 0; i < 7; i++) ch.tick(4);
      // dutyPos = 7 → HIGH for duty 0.
