 ...
 *
 * Persisted in IndexedDB keyed by {@link cartIdOf}, mirroring the
 * save-RAM / save-states / cheats stores — patched ROMs get their own
 * overrides independent of the vanilla base.
 */
export interface CartOverrides {
   ...
   *  Twisted where the user wants to fill the frame; on is the default
   *  for sharp pixel preservation. */
...
/** Load overrides for a cart. Returns an empty object if none are
 *  persisted — the caller treats "no override" and "no record" the
 *  same way, so null wouldn't add signal. */
export async function loadCartOverrides(cart: Cartridge): Promise<CartOverrides> {
  try {
    const rec = await idbGet<CartOverridesRecord>(STORE_CART_OVERRIDES, cartIdOf(cart));
 ...
 *  entries over time. */
export async function saveCartOverrides(cart: Cartridge, overrides: CartOverrides): Promise<void> {
