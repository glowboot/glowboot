...
document.addEventListener("visibilitychange", () => {
  if (document.visibilityState === "hidden" && state.gb) {
  ...
  } else if (document.visibilityState === "visible" && state.gb) {
    if (!state.paused && state.rtcWallPauseMs > 0) {
      state.gb.cart.advanceRtcByWallMs(Date.now() - state.rtcWallPauseMs);
      state.rtcWallPauseMs = 0;
    }
    if (!state.paused) startPlayTimer();
  }
});
