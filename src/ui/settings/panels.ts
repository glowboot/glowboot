...
type RenderMode = "webgl-bilinear" | "canvas" | "webgl-crt" | "webgl-hq2x" | "webgl-lcd" | "webgl-mmpx" | "webgl-sxbr";
...
// ─── Audio mode ──────────────────────────────────────────────────────────
// Filter chain installed between the master gain and the destination.
// "studio" passes audio through unchanged (flat reference monitors);
// "gb-speaker" emulates the handheld's tiny mono driver via a band-pass
// + soft-clip; "warm-headphones" applies a gentle line-out EQ; etc.
// Persists in `gb-audio-mode`.
{
  const DEFAULT_AUDIO_MODE: AudioMode = "studio";
  const validIds = new Set<string>(AUDIO_MODES.map((m) => m.id));
  const stored = INIT_LS[KEYS.AUDIO_MODE] ?? DEFAULT_AUDIO_MODE;
  const initial = (validIds.has(stored) ? stored : DEFAULT_AUDIO_MODE) as AudioMode;
  audio.setAudioMode(initial);
  const el = audioModeSelect;
  if (el) {
    el.innerHTML = "";
    ...
    el.value = initial;
    el.addEventListener("change", () => {
      if (!validIds.has(el.value)) return;
      const v = el.value as AudioMode;
      audio.setAudioMode(v);
      lsSet(KEYS.AUDIO_MODE, v);
    });
  }
  onSectionReset("audio", () => {
    audio.setAudioMode(DEFAULT_AUDIO_MODE);
    if (el) el.value = DEFAULT_AUDIO_MODE;
  });
}
...
wireSectionReset(audioResetBtn, "audio", [KEYS.VOLUME, KEYS.CHANNEL_MUTES, KEYS.AUDIO_MODE], "Audio");
wireSectionReset(
  sessionResetBtn,
  "behavior",
