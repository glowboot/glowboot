 ...
 * per Game Boy button. Click a chip to enter capture mode — the next
 * keypress or gamepad input (button / axis / POV-hat sector) is recorded
 * as the new binding. Capture cancels on Escape or by clicking a
 * different chip.
 *
 ...
 *
 * Keyboard bindings are published through the `codeToButton` live-binding
 * export so `keyboard.ts` picks up re-bindings immediately.
 */
...
function renderBindingsKeyboard(): void {
  ...
  // keyboard-only inputs; only an MBC7 cart (Kirby Tilt 'n' Tumble,
  // Command Master) actually consumes them, otherwise the keys are
  // inert. Render them after the GB buttons so the section reads
  // top-to-bottom: D-pad → A/B → Start/Select → Tilt.
...
function startGamepadCapture(gb: Button, chip: HTMLElement): void {
  ...
  const cancelPad = gamepad.captureNext((binding) => {
    const sameAs = (a: Bindings.GamepadBinding, b: Bindings.GamepadBinding): boolean => {
    ...
    const prevOwner = Bindings.GB_BUTTONS.find((g) => {
      ...
      return other !== null && sameAs(other, binding);
    });
